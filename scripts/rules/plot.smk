rule prepare_data_for_plot:
    ...
    output:
        expand(f"{config['BUILD_DIR']}/prepared_{{format}}/{{key}}", format=["time", "size"], key=["alpha", "hash", "kmer", "relaxed-fpr", "none", "U", "U+R"]),
    threads: config["NUM_THREADS"]
    log: "log/prepare_data_for_plot.log",
...
rule plot_data:
    ...
    threads: config["NUM_THREADS"]
    log: "log/plot_data.log",
    conda: "../envs/bokeh_env.yaml"
    shell:
        """
